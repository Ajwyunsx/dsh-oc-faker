...
function defaults(config) {
  ...
  return {
    ...
    zenHeaders: cfg.zenHeaders !== false,
    zenVersion: typeof cfg.zenVersion === 'string' && cfg.zenVersion ? cfg.zenVersion : '1.18.16',
    zenForcePublic: cfg.zenForcePublic === true,
  }
}
  ...
  try {
    if (existsSync(STATE_FILE)) {
      ...
      return {
        ...
        zenHeaders: j.zenHeaders !== false,
        zenVersion: typeof j.zenVersion === 'string' && j.zenVersion ? j.zenVersion : d.zenVersion,
        zenForcePublic: j.zenForcePublic === true,
      }
    }
  } catch { /* 损坏则回落默认 */ }
...
// ── OpenCode Zen 官方客户端头组 ──
// zen 免费池（-free 模型）识别的是整套 opencode 客户端头：UA 只是其一，
// 官方 TUI 还会发 x-opencode-client / x-opencode-session / x-opencode-project /
// x-opencode-request（每次随机 ID），匿名免费池用 Bearer public。伪装 UA 不够。
const ZEN_HOST_SUFFIX = '.opencode.ai'

function isZenHost(host) {
  const h = String(host || '').toLowerCase()
  return h === 'opencode.ai' || h.endsWith(ZEN_HOST_SUFFIX)
}

// 判定一个 Authorization 值是否为「哑认证」应被清空：
// - 无 auth / 空 Bearer / Bearer public → 哑认证（免费池无 key 可走）
// - 乱填/过短/非 sk- 形态的 token → 哑认证（opencode 会回 401 Invalid API key）
// - 形如 sk-xxx…（大于等于 32 字符）的 token → 视为真付费 key，保留
function isDummyAuthValue(auth) {
  if (!auth) return true
  const m = /^Bearer\s*(\S*)$/i.exec(String(auth))
  const token = m ? m[1] : ''
  if (!token || token === 'public') return true
  return !(/^sk-/i.test(token) && token.length >= 32)
}

function applyZenHeaders(state, headers) {
  try {
    if (state.zenHeaders === false) return false
    headers.set('user-agent', 'opencode/' + (state.zenVersion || '1.18.16'))
    headers.set('x-opencode-client', 'cli')
    headers.set('x-opencode-project', 'global')
    headers.set('x-opencode-session', 'ses_' + Math.random().toString(36).slice(2, 12))
    headers.set('x-opencode-request', 'msg_' + Math.random().toString(36).slice(2, 12))
    // opencode：免费模型无需 key；乱填/空/public 会被回 401 Invalid/Missing API key。
    // 传输层兜底：凡不是「sk- 开头且 >=32 字符」的真 key 形态，一律清空 Authorization，
    // 让免费模型请求走无 key 通道；真付费 key 保留（不吞用户 key）。
    const existingAuth = headers.get('authorization');
    if (isDummyAuthValue(existingAuth)) {
...
function installFetchWrapper(state) {
  ...
  const wrapper = async function uaSpoofFetch(input, init) {
    ...
    if (isZenHost(host) && !matchList(host, state.keep)) {
      applyZenHeaders(state, headers)
    }
...
function normalizeHeaderEntries(headers) {
  ...
  if (headers && typeof headers === 'object') {
    ...
    return Object.entries(headers).map(([k, v]) => [String(k).toLowerCase(), String(v)])
  }
  return []
}
...
function installDispatcherWrap(state) {
  ...
  const wrapped = function uaSpoofDispatch(origin, options) {
    ...
    if (options && options.headers !== undefined) {
      ...
      if (isZenHost(host) && !matchList(host, state.keep) && state.zenHeaders !== false) {
        const zen = entries.filter(([k]) => !['user-agent', 'authorization', 'x-opencode-client', 'x-opencode-session', 'x-opencode-project', 'x-opencode-request'].includes(k))
        zen.push(['user-agent', 'opencode/' + (state.zenVersion || '1.18.16')])
        const existingAuth = entries.find(([k]) => k === 'authorization')?.[1]
        if (!isDummyAuthValue(existingAuth)) {
          zen.push(['authorization', existingAuth])
        }
        zen.push(['x-opencode-client', 'cli'])
        zen.push(['x-opencode-project', 'global'])
        zen.push(['x-opencode-session', 'ses_' + Math.random().toString(36).slice(2, 12)])
        zen.push(['x-opencode-request', 'msg_' + Math.random().toString(36).slice(2, 12)])
        opts = { ...options, headers: zen }
      }
    }
...
export function apply(ctx, config) {
  ...
  const summary = () => [
    ...
    `zen(headers+public): ${state.zenHeaders !== false ? 'on v' + (state.zenVersion || '1.18.16') : 'off'}`,
    `zenForcePublic: ${state.zenForcePublic ? 'on（乱填 key 自动归一 Bearer public，跳过付费校验）' : 'off（保留原 key）'}`,
  ...
  registerTool({
    ...
    parameters: {
      ...
      zenHeaders: { type: 'number', description: '1 = inject full opencode client headers on opencode.ai (zen); 0 = UA only' },
      zenVersion: { type: 'string', description: 'opencode version string for zen UA, e.g. 1.18.16' },
      zenForcePublic: { type: 'number', description: '1 = force Bearer public on opencode.ai (zen ignores garbage keys with 401; free pool needs public/none); 0 = keep original key' },
    },
    execute(args) {
      ...
      if (args.zenHeaders === 1) state.zenHeaders = true
      if (args.zenHeaders === 0) state.zenHeaders = false
      if (args.zenForcePublic === 1) state.zenForcePublic = true
      if (args.zenForcePublic === 0) state.zenForcePublic = false
      if (typeof args.zenVersion === 'string' && args.zenVersion.trim()) state.zenVersion = args.zenVersion.trim()
